...
export const Vec3Editor: FunctionalComponent<Props> = ({
    value,
    onChange,
    disabled = false,
}) => {
    const [showMenu, setShowMenu] = useState(false);
    const [vecInternal, setVecInternal] = useState<string[]>(["0", "0", "0"]);
    ...
    useEffect(() => {
        if (!showMenu) {
            if (Array.isArray(value)) {
                setVecInternal(value.map((v: any) => String(v)));
            } else {
                setVecInternal(["0", "0", "0"]);
            }
        }
    }, [value, showMenu]);
    ...
    const cancel = () => {
        setVecInternal(Array.isArray(value) ? value.map(String) : ["0", "0", "0"]);
        setShowMenu(false);
    };
    ...
    return (
        <div className="node-variable-editor-vec3" ref={containerRef} onPointerDown={handlePointerDown as any}>
            <div className="node-variable-editor-pill" onClick={openMenu as any} onPointerDown={(e) => e.stopPropagation()}>
                {Array.isArray(value) ? `${value[0]}, ${value[1]}, ${value[2]}` : "0, 0, 0"}
            </div>
            ...
            {showMenu && createPortal(
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9998 }}>
                    ...
                    <div className="node-color-popover" style={{ top: popoverPosition.y, left: popoverPosition.x }} onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
                        ...
                        <div className="node-color-row">
                            <div className="node-color-label">Z</div>
                            <input className="node-color-input" value={vecInternal[2]} onInput={(e) => handleVecChange(2, (e.target as HTMLInputElement).value)} onKeyDown={handleKeyDown as any} />
                        </div>
